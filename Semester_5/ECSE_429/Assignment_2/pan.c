...
short src_ln1 [] = {
	  0,  33,  33,  34,  34,  32,  37,  38, 
	 39,  40,  41,  42,  43,  43,  37,  46, 
	 46,  30,  48,  30,  48,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "A2_Q3.pml", 1, 20 },
...
{	int j, h = now._nr_pr;
	...
	case 2:	/* client */
		...
		/* locals: */
#ifdef VAR_RANGES
		logval("client:id", ((P2 *)pptr(h))->id);
#endif
#ifdef HAS_CODE
		locinit2(h);
#endif
		break;
	case 1:	/* round_robin_arbiter */
		((P1 *)pptr(h))->_t = 1;
		((P1 *)pptr(h))->_p = 17;
#ifdef HAS_PRIORITY
		...
		((P1 *)pptr(h))->i = 0;
#ifdef VAR_RANGES
		logval("round_robin_arbiter:i", ((P1 *)pptr(h))->i);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
...
iniglobals(int calling_pid)
{
	...
	{	int l_in;
		for (l_in = 0; l_in < 4; l_in++)
		{
			logval("req[l_in]", now.req[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 4; l_in++)
		{
			logval("gnt[l_in]", now.gnt[l_in]);
		}
	}
		logval("pot", now.pot);
#endif
...
{	/* int i; */
	...
	printf("	byte   id:	%d\n", ((P2 *)pptr(pid))->id);
		break;
	case 1:
		printf("local vars proc %d (round_robin_arbiter):\n", pid);
	printf("	short  i:	%d\n", ((P1 *)pptr(pid))->i);
		break;
	case 0:
		/* none */
